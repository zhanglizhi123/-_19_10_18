...
- (void)moveIndicatorView:(BOOL)animated
{
    ...
    /**
     *
     下面 self.indicatorHeight是自己添加的 原来是2
     */
    [UIView animateWithDuration:(animated?0.05:0) animations:^{
        switch (_indicatorType) {
            case FSIndicatorTypeDefault:
                self.indicatorView.frame = CGRectMake(selectBtn.frame.origin.x , CGRectGetHeight(self.scrollView.bounds) - self.indicatorHeight, CGRectGetWidth(selectBtn.bounds), self.indicatorHeight);
                break;
            case FSIndicatorTypeEqualTitle:
                self.indicatorView.center = CGPointMake(selectBtn.center.x, CGRectGetHeight(self.scrollView.bounds) - self.indicatorHeight/2.0);
                self.indicatorView.bounds = CGRectMake(0, 0, indicatorWidth, self.indicatorHeight);
                break;
            case FSIndicatorTypeCustom:
                self.indicatorView.center = CGPointMake(selectBtn.center.x, CGRectGetHeight(self.scrollView.bounds) - self.indicatorHeight/2.0);
                self.indicatorView.bounds = CGRectMake(0, 0, indicatorWidth + _indicatorExtension*2, self.indicatorHeight);
                break;
